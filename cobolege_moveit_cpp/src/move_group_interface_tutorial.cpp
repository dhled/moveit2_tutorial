// Based on the movegroup tutorial from moveit2 - Tuned for cobolege robot
...
int main(int argc, char** argv)
{
  ...
  // We define the frame/pose for this cylinder so that it appears in the torch link (cobolege).
  ...
  // We also need to tell MoveIt that the object is allowed to be in collision with the links.
  // You could also use applyAttachedCollisionObject to attach an object to the robot directly.
  RCLCPP_INFO(LOGGER, "Attach the object to the robot");
  std::vector<std::string> touch_links;
  touch_links.push_back("tcp_torche");
  move_group.attachObject(object_to_attach.id, "tcp_torche", touch_links);
  ...
  // ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  //
  // Now, let's detach the cylinder from the robot's torche.
